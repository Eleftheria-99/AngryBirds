...
public class ABObject extends Rectangle {
	...
	/**
	 * @return true if this touches with obj, false otherwise; the enlarged area is
	 *         used
	 */
	public boolean touches(ABObject obj) {
		...
		Rectangle mbr = (Rectangle) obj.getBounds().clone();
		mbr.grow(shift, shift);
		_shiftar=new Area(mbr);
		Area tmp = (Area) _shiftar.clone();
		tmp.intersect(_shiftar);
	...
	/**
	 * |-OBJ-----------| |left| |-OBJ-----------| |Left||-OBJ-----------| |support|
	 * |left| |directlybelow| |below| |below a not left!|
	 */
	 ...
	 * objs refers to a list of potential supporters. Empty list will be returned if
	 * no such supporters.
	 */
	...
	/**
	 * finds all objects, which recursively touches | A || B || C || D | - a touches
	 * b touches c touches d = a touches d
	 */
